...
<div class="py-5" style="min-height: 80vh;">
    <div class="container-xl">
        ...
        <nav aria-label="breadcrumb" class="mb-4">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ url('/') }}" class="text-decoration-none text-white-50">Home</a></li>
                <li class="breadcrumb-item"><a href="{{ route('community.index') }}" class="text-decoration-none text-white-50">Community Meetups</a></li>
            ...
            <div class="col-lg-8">
                <div class="static-card mb-4 h-auto">
                    <div class="pb-4 mb-4">
                        ...
                        <p class="text-white-50 mb-0"><i class="bi bi-clock me-1"></i> Posted {{ $meetup->created_at->diffForHumans() }}</p>
                    </div>

                    <div>
                        <h4 class="section-heading fs-5 mb-3">About this Meetup</h4>
                        <div class="section-subtext mb-5" style="white-space: pre-wrap;">{{ $meetup->description }}</div>
                        
                        <div class="row g-3">
                            <div class="col-sm-6">
                                <div class="d-flex align-items-center p-3 rounded-3 h-100" style="background: rgba(255,255,255,0.05);">
                                    <div class="rounded-circle d-flex align-items-center justify-content-center text-primary me-3" style="width: 48px; height: 48px; background: rgba(var(--theme-color-rgb), 0.1);">
                            ...
                            <div class="col-sm-6">
                                <div class="d-flex align-items-center p-3 rounded-3 h-100" style="background: rgba(255,255,255,0.05);">
                                    ...
                                    <div>
                                        ...
                                        @endphp
                                        <div class="fw-bold">{{ $approvedCount }} / {{ $meetup->headcount_limit ?? 'Unlimited' }}</div>
                ...
                <div class="static-card mb-4 text-center h-auto">
                    <div>
                        <div class="mb-3 position-relative d-inline-block">
                            <div class="avatar avatar-xl bg-primary text-white rounded-circle d-flex align-items-center justify-content-center fw-bold mx-auto shadow-sm" style="width: 80px; height: 80px; font-size: 2rem;">
                                {{ substr($meetup->user->name, 0, 1) }}
                            </div>
                            @if($meetup->user->is_verified)
                                <span class="position-absolute bottom-0 end-0 bg-success text-white border border-2 border-white rounded-circle p-1 d-flex align-items-center justify-content-center" style="width: 24px; height: 24px;" title="Verified User">
                        ...
                        <div class="d-flex justify-content-center align-items-center gap-2 mb-4">
                            ...
                            <div class="px-3 py-2 rounded-3" style="background: rgba(255,255,255,0.05);">
                                <div class="fw-bold"><i class="bi bi-star-fill text-warning me-1"></i>{{ number_format($meetup->user->rating ?? 0, 1) }}</div>
                                <div class="small text-white-50" style="font-size: 0.7rem;">Rating</div>
                            </div>
                        </div>

                        <a href="{{ route('profile.view', ['id' => $meetup->user->id]) }}" class="btn-theme-outline-secondary w-100 justify-content-center">
                        ...
                        @auth
                            @if(auth()->id() == $meetup->user_id)
                                <div class="alert alert-info rounded-3 mb-0">
                                    <i class="bi bi-info-circle me-2"></i> You are the host of this meetup. Manage it from your <a href="{{ route('meetups.my') }}" class="alert-link">dashboard</a>.
                                </div>
                            @else
                                @php
                                ...
                                @if($existingRequest)
                                    @if($existingRequest->status == 'pending')
                                        <div class="alert alert-warning rounded-3 mb-0">
                                            <i class="bi bi-hourglass-split me-2"></i> Your request to join is pending approval from the host.
                                        </div>
                                ...
                                @elseif($meetup->status != 'open')
                                    <div class="alert alert-secondary rounded-3 mb-0">
                                        <i class="bi bi-slash-circle me-2"></i> This meetup is currently {{ $meetup->status }} and not accepting new requests.
                                    </div>
                                @else
                            ...
                            @endif
                        @else
                            <div class="alert rounded-3 text-center mb-3" style="background: rgba(255,255,255,0.05); color: inherit;">
