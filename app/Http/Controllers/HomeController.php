...
class HomeController extends Controller
{
    ...
    {
        // 1. Sponsored Hero Carousel Ads (is_sponsored = true or top views)
        ...
        // 2. Trending Near You Section (Ordered by views / engagement)
        ...
        // 3. Featured Listings Section (Promoted Marketplace Inventory with is_featured = true)
        $featuredModels = Listing::with(['category', 'primaryImage'])
            ->where('status', 'active')
        ...
        $locations = Listing::select('city', 'province', \DB::raw('COUNT(*) as listings_count'))
            ...
            ->map(function ($loc, $index) {
                return [
                    ...
                    'slug'           => Str::slug($loc->city . '-' . $loc->province),
                ];
            });
